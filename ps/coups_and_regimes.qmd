...
This study employs a sample selection model [@heckman1979] to assess the
probability of coup attempts and their likelihood of success. The most
appropriate model for this research is the two-stage `probit` model, as detailed
by @vandeven1981 and utilized by @powell2012. The first stage examines the
probability that a regime experiences a coup attempt, while the second stage
evaluates the probability of the attempt's success.
...
To test our argument, we use the coup attempts and outcomes from @powell2011 as
the dependent variable. Successful coups are those in which the incumbent is
removed from the office for over seven days. Data are available from 1950 to
2023, accounting for 491 coup attempts, of which 245 succeeded. The coup
descriptive statistics have been discussed in previous part.

#### Independent variables

The main independent variable, as analysed, is the regime type. We follow the
category from @geddes2014 (GWF). Our main concern focuses on the military,
personal and dominant-party regimes, while democracy and monarchy will be
included as references. The descriptive statistics is shown in @tbl-regimes.

#### Control variables

**Economic performance:** we use the current‐trend (CT) ratio explored by
@krishnarajan2019 to measure the economic performance. This measurement
calculates the ratio between the\
current level of gross domestic product per capita (GDP/cap) and the average\
GDP/cap level of the previous 5 years. For a country 𝑖 at year 𝑡 the approach is\
given as follow:
...
Since a given year's situation is mainly determined by the previous, the variable
is lagged for one year. The GDP per capita (in constant 2017 international
dollars, PPP) uses the vdem dataset which comes from @fariss2022.

**Violence:** Violence is an index which captures all kinds of violence and wars
(internal or\
interstate). The dataset is from Major Episodes of Political Violence
[@marshall2005current].

**Military expending:** Military expending per capita measure how much money is
expended per capita per year in a regime. The data source is the Correlates of
