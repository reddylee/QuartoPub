...
abstract: |
  Previous research on the determinants of coup attempts has predominantly focused on the factors or conditions present when or before coups are launched, with less emphasis on the outcomes following the coups. However, the results of post-coup events play a crucial role in the decision-making process of coup attempts, either attracting potential plotters with significant benefits or deterring them with substantial costs. Inspired by the observed success rates of coups, this study posits that the expected outcomes of a coup are pivotal determinants of coup attempts. Utilizing a sample selection model, specifically a two-stage probit model, this research aims to elucidate why coups are more prevalent in certain autocratic countries while being rare in others. The findings suggest that, influenced by the distinct power dynamics between coup perpetrators and incumbents, coups are more likely to occur in military and personalist regimes compared to dominant-party regimes.
...
the determinants of coups, it is crucial not to overlook the most significant
characteristic of coups themselves. As defined by @powell2011, coups are "illegal
...
with immature conditions and low chances of success will be filtered out.
Fundamentally, launching a coup is not like participating in a general election,
where failure in one election allows for another attempt in the next election. A
coup is a high-stakes gamble where success brings substantial rewards, while
failure may result in martyrdom. Failure, put differently, means permanent and
absolute defeat. Furthermore, the outcomes of coups are typically decided within
a matter of days, sometimes even mere hours. Unlike prolonged conflicts where
victories or defeats may shift over time, the success of a coup is often sealed
at its inception. Every detail must be meticulously planned and arranged
beforehand.
...
The subsequent section of this paper explores the dynamics of coup attempts and
...
country. In a two-stage sample selection model, the second-stage model cannot be
identical to the first-stage model. Therefore, the first-stage model includes the
number of previous coups, while the second-stage does not. Previous coups may
have a greater impact on coup attempts, while once a coup is launched, the number
of previous coups might not significantly explain the outcome.
...
stargazer(model_selection, model_2step,
          ...
          label = "results",
          covariate.labels = c("Constant", "Democracy", "Regime: Military", "\\hspace{1.6cm}Monarchy", "\\hspace{1.6cm}Personalist", "Economic trend", " GDP per capita", "Political violence",  "Previous coups"),
          dep.var.caption = "",
...
The findings from the two-stage probit model, as presented in Table
\ref{results}, provide valuable insights into the dynamics of coup attempts and
their likelihood of success across different regime types:
...
stage, which removes the influence of these variables on the coup outcome.
Interestingly, higher GDP per capita shows a positive relationship with the
likelihood of coup success at the 5% significance level. This suggests that
better economic conditions may provide more resources to coup plotters, thereby
increasing the chances of a successful coup once attempted. Conversely, weak
economies may trigger more coup attempts, though many of these are less likely to
succeed.
...
-   Better economic conditions, while reducing the likelihood of coup attempts,
    increase the likelihood of coup success.
...
-   The Heckman two-step model is designed to correct for selection bias. The
    similar success rates of coups across different regimes, despite varying
    attempt rates, indicate potential selection bias. This supports the need for
    a sample selection model, as coup plotters are likely to attempt coups only
    when they perceive a high likelihood of success.
...
determining the likelihood of coup attempts. Through the application of a
two-stage sample selection model, this research highlights that coups are
