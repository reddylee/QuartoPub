...
and a long-term vision, further bolstering stability. Unlike typical political
parties, dominant parties often rise to power through revolutions or coups,
resulting in a robust structure with no single individual wielding absolute
power. The military force aligns with the party itself, not individual leaders,
contributing significantly to greater regime stability compared to personalist or
military regimes.

Personalist regimes exhibit a degree of temporary stability. Dictators, often
ruthless and battle-hardened, emerge from intense competition. While lacking
strong institutions, purges create a temporary status quo. However, the lack of a
clear successor creates a vulnerability during leadership transitions. When
dictators die or are assassinated, the absence of an established succession plan
can trigger coups d'état.

Military regimes tend to be the least stable, often ruling through a junta.
Juntas lack a unified vision and are prone to internal power struggles, hindering
stability. For example, General Augusto Pinochet's rise to power in Chile after
the 1973 coup demonstrates the inherent instability of juntas. While his
colleagues initially saw him as a safe choice, his actions proved otherwise. This
lack of cohesion within juntas makes them particularly susceptible to internal
fracturing.

The strength of political institutions further supports this assessment. Studies
by @frantz2017 show that 97% of country-years in dominant-party regimes have
formalized institutional succession rules, compared to 77% in personalist regimes
and a mere 59% in military regimes.

These differing power dynamics significantly impact the likelihood of coup
attempts. According to @tbl-regimes, military regimes face the highest risk,
representing only 5.6% of country-years yet accounting for over 22% of coups.
Personalist regimes follow, with 23% of coups despite comprising only 13% of
country-years. Dominant-party regimes, with their stronger institutions and
unified leadership, have the lowest incidence of coups – constituting 22.6% of
country-years but responsible for only 16.7% of coups.
...
regime_total <- regimes |>
  summarise(country_year = n(), .by = regime_type) |>
  arrange(desc(country_year)) |>
  mutate(
    share_of_country_year = country_year / sum(country_year),
    share_of_country_year = percent(share_of_country_year)
  ) |>
...
coup_by_regime <- regimes |>
  ...
  filter(!is.na(coup)) |>
  summarise(no_of_coups = n(), .by = regime) |>
  mutate(
    percent_of_coups = no_of_coups / sum(no_of_coups),
    percent_of_coups = percent(percent_of_coups)
  ) |>
  adorn_totals(where = "row", fill = "100%")
