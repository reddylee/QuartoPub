...
difference places the two types of leaders at varying levels of disadvantage in
...
In conclusion, coup-entry leaders typically face a greater degree of the
challenges discussed earlier, placing them at a significant disadvantage in the
balance of power compared to autocoup leaders. This disadvantage creates a
self-perpetuating cycle. Weaker leaders struggle to attract and retain strong
support, making them more vulnerable to challenges. The perception of risk
discourages potential allies, further eroding their power base. As shown in
@tbl-coups, the top 10 countries with the most coup attempts between 1950 and
2023 are predominantly autocracies. This suggests that the more coups occur in a
...
The GDP per capita data (in constant 2017 international dollars, PPP) is sourced
from the V-Dem dataset by @fariss2022. To capture the prior year's economic
impact, the data is lagged by one year.
...
-   **Degree of Democracy:** The level of democracy is gauged using Polity 5
    scores (polity) at the entry year for each respective country. These scores
    range from -10 (fully autocratic) to +10 (fully democratic), capturing the
    extent of democratic versus autocratic governance. This dataset is sourced
    from the Center for Systemic Peace (CSP)[^4] and provides an essential
    measure of political regime type, which can influence the stability and
    survival of leaders in power.

-   **Population Size:** To account for its potential impact on leaders' tenures,
    the log of the population size (`pop_log`) is considered. This transformation
    helps in managing the wide range of population sizes across different
    countries. The data is sourced from the V-Dem dataset and is evaluated to
    understand its influence on leadership survival. Larger populations may
    present more governance challenges and potential sources of opposition,
    thereby affecting the stability and longevity of a leader's tenure.
...
Building on the preceding discussion, I investigated my hypothesis regarding the
likelihood of prolonged regime survival among leaders who overstay their terms
compared to those who assume power through coups. The regression results of both
the Cox PH model and the Time-dependent Cox model are presented in @tbl-cox.
...
It is evident from the table that both models produce comparable outcomes,
indicating that the type of leader indeed significantly influences the survival
duration of their regime, even when controlling for other pertinent factors. Here
I use the Time-dependent Cox model to illustrate the results.

The hazard ratio for coup-entry leaders is 2.41, signifying that coup-entry
leaders are 1.41 times more likely to face ousting compared to overstaying
leaders, all else being equal.

Additionally, the hazard ratio associated with the logarithm of GDP per capita is
0.73, suggesting that a 1% increase in GDP per capita is correlated with a 27%
reduction in the hazard of being ousted, or 1% decrease in GDP per capita is
correlated with a 37% reduction in the hazard of being ousted.

It is noteworthy that the remaining variables did not exhibit statistical
...
Mydata1 |>
  mutate(group = factor(group),
         survt = tstop - tstart) |>
  coxph(
    Surv(survival_time, status) ~ group + GDP_trend + GDP_pc + pop_log + polity5 + polstab + age,
    data = _
  ) |> 
...
Mydata2 |>
  mutate(group = factor(group),
         survt = tstop - tstart) |>
  coxph(
    Surv(T1, T2, status) ~ group + GDP_trend + GDP_pc + pop_log + polity5 + polstab + age + cluster(id),
    data = _
  ) |> 
...
The survival curves depicted in @fig-coxSurv illustrate the respective survival
rates for leaders of both types. Once more, both the Cox proportional hazards
(PH) model and the Time-dependent model generate analogous plots. Notably, the
survival curve for leaders who enter through coups exhibits a significantly lower
trajectory compared to those who overstay their terms. This disparity suggests
that overstaying leaders are more inclined to endure for longer durations than
...
hazards (PH) model and the Time-dependent model yield nearly identical plots.
Notably, the hazard ratio for leaders who assume power through coups and the
logarithm of GDP per capita emerge as statistically significant factors.

## Conclusion

Adipiscing netus morbi facilisis parturient nunc habitant. Mauris penatibus
taciti penatibus nisi aliquet himenaeos, auctor lobortis tellus. Metus eget
ornare nullam quisque primis nec ornare bibendum auctor. Nunc tempus natoque
phasellus senectus pellentesque hendrerit, turpis hac vel odio fames, suscipit
quisque egestas eros quis.

Adipiscing rutrum, enim est eu, luctus; leo tristique posuere taciti, iaculis
quisque torquent! Torquent phasellus dis aliquam maecenas aptent himenaeos a duis
semper fringilla fringilla. Arcu volutpat nam sociis nullam sociosqu tristique eu
habitant. Pellentesque lobortis taciti sodales, habitant posuere massa nascetur
velit eleifend.

Ipsum sociis; risus torquent molestie senectus porttitor, pharetra magna ad
rutrum convallis. Imperdiet eleifend habitant sociis, aliquet mi, dui himenaeos
dictum fames? Dis sem ut, vel; lacus tortor duis aptent sociosqu litora cras
nisi? Nam conubia in vivamus porttitor taciti?

Elit vitae leo ante orci, erat vestibulum primis elementum laoreet? Ante massa
condimentum pellentesque odio ligula velit fringilla. Hac habitant ullamcorper
interdum phasellus, pulvinar orci mi, convallis gravida odio taciti. Elementum
ligula per donec fermentum class duis nisi ultrices?

Consectetur maecenas sociosqu, netus ad nisi, class urna aptent convallis
imperdiet nostra tellus. Hendrerit himenaeos sagittis sapien venenatis, eu
senectus varius lectus. Condimentum dictum arcu ornare lacinia ornare lectus
magna! Inceptos tempus tempus imperdiet suspendisse mattis enim accumsan quisque,
proin ridiculus congue, nulla cursus primis.
