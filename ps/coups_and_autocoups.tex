...
\ref{results}). Models 1 uses the numbers of previous coups, while model
2 uses the time since the last coup.
...
While it is not intuitive to interpret the results of probit model,
Table~\ref{tbl-mfx1}, using a marginal effect package
(\citeproc{ref-marginaleffects}{Arel-Bundock, Greifer, and Heiss NaN}),
helps us to understand the regime effect more clearly. Column AME shows
the marginal effect in terms of changes in probability, which means how
much probability increase when the variable increases 1 unit( numeric
variables) or compared to reference (categorical variables). For
example, military's marginal effect is 0.07, means probability of coup
attempts in military regimes is 7 percentage points (pp) higher than in
dominant-party regimes, holding other factors constant. GDP per capita's
marginal effect is -0.002, means the probability of coup attempts in
average condition will decrease by 0.2 pp when GDP per capita increase 1
unit, i.e., 1000 dollars in this case.

Ratio column shows the change ratio in percent. It tells the relative
increase or decrease. While a 7 pp increase might initially seem modest,
it is substantial in context. The average probability of a coup attempt
in dominant-party regimes is only about 2.53\%. Therefore, a 7 pp
increase means that military regimes are approximately 277.7\% more
likely to encounter coups than dominant-party regimes, which represents
a significant and substantial change. Similarly, personalist regimes are
2.4 pp higher, about 94\% more likely compared to dominant-party, which
is substantial as well. Monarch regimes also display a positive effect
similar to personalist regimes, reflecting the fact that monarchies are
a subset of personalist regimes with royal titles.

However, effects from control variables are weak in predicting coup
attempts. None of their marginal effect of probability reach 1 pp, and
less than 7\% in ratio. Given the actually probability is only 2\% to
3\%, less than 7\% increase or decrease can be ignored.
...
Model 2 uses the years since last coup instead of number of previous
coups. Since most countries did not experience coups, I use an
interactive term of previous coups (Notice: in Model 2, previous coups
is a dummy variable of with coup or without coup) and years since last
coup. Generally speaking, model 2 shows the same direction results, with
relatively lower coefficients than model 1.
...
\includegraphics{coups_and_autocoups_files/figure-pdf/fig-margins-1.pdf}
